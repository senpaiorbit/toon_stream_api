...
async function scrapeCastPage(baseUrl, castName, pageNumber = 1) {
  try {
    // Try both URL patterns: /home/cast_tv/ and /cast/
    let castUrl;
    const castSlug = castName.toLowerCase().replace(/\s+/g, '-');
    
    if (pageNumber === 1) {
      castUrl = `${baseUrl}/home/cast_tv/${castSlug}/`;
    } else {
      castUrl = `${baseUrl}/home/cast_tv/${castSlug}/page/${pageNumber}/`;
    }
    
    console.log(`Scraping: ${castUrl}`);
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Get cast name from query parameter
    const castName = req.query.cast;
    
    if (!castName) {
      return res.status(400).json({
        success: false,
        error: 'Cast name is required. Use ?cast=saori-hayami'
      });
    }
    ...
    const result = await scrapeCastPage(baseUrl, castName, pageNumber);
