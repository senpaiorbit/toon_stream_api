...
// Parse seasons query parameter - supports [1,2,3], [1-3], or "all"
...
// Scrape series metadata from main series page using proxy
async function scrapeSeriesMetadata(baseUrl, seriesSlug) {
  const path = `series/${seriesSlug}/`;
...
// Scrape episode servers using proxy
async function scrapeEpisodeServers(episodeSlug, serverQuery) {
  try {
    const path = `episode/${episodeSlug}/`;
    const html = await fetchViaProxy(path);
    const $ = cheerio.load(html);
    const servers = [];
    let serverIndex = 0;
...
// Scrape episodes for a specific season using proxy
async function scrapeSeasonEpisodes(seriesSlug, seasonNumber, includeSrc, serverQuery) {
  const path = `episode/${seriesSlug}-${seasonNumber}x1/`;
  ...
  try {
    ...
    if (includeSrc && seasonData.episodes.length > 0) {
      ...
      for (const episode of seasonData.episodes) {
        ...
        if (episodeSlug) {
          episode.servers = await scrapeEpisodeServers(episodeSlug, serverQuery);
        }
      }
    }
...
async function scrapeSeriesPage(baseUrl, seriesSlug, seasonsQuery, includeSrc, serverQuery) {
  try {
    ...
    for (const seasonNum of validSeasons) {
      ...
      const seasonData = await scrapeSeasonEpisodes(seriesSlug, seasonNum, includeSrc, serverQuery);
  ...
  try {
    ...
    if (!seriesSlug) {
      return res.status(400).json({ 
        success: false, 
        error: 'Series slug required. Examples: ?slug=naruto&seasons=[1,2,3] or ?slug=naruto&seasons=[1-3] or ?slug=naruto&seasons=all or ?slug=naruto&seasons=latest&src=true&server=[0,1,2]' 
      });
    }
